...
export default function ReservationsPage() {
  ...
  const calculateAvailableStartTimes = useCallback(
    (
      ...
      const daySchedule = timetable.filter(
        (entry) => entry.dayOfWeek === dayOfWeek && entry.type === 'OPEN',
      );
        ...
        while (currentMins + requiredDuration <= endMins) {
          ...
          const hardwareCount = existingReservations.filter(
            (r) =>
              ...
              timeToMins(r.startTime) < endMinsCalc &&
              timeToMins(r.endTime) > currentMins,
          ).length;
          ...
          if (inventoryType === 'switch') {
            ...
            controllersNeeded =
              inventoryType === 'ps5' ? controllers : extraController ? 1 : 0;
          ...
          if (
            hardwareCount < maxHardware &&
            (controllersNeeded === 0 ||
              controllersInUse + controllersNeeded <= maxControllers)
          ) {
  ...
  const availableStartTimes = useMemo(() => {
    return calculateAvailableStartTimes(
      formData.date,
      formData.duration || '60',
      formData.inventory,
      formData.controllers,
      formData.extraController,
    );
  }, [
    ...
    try {
      ...
      const res = await apiClient.POST('/reservations', {
        body: {
          ...
          controllers:
            formData.inventory === 'pc' && formData.extraController
              ? 1
              : formData.controllers,
        },
      });
    ...
    <div className='min-h-screen bg-slate-950 py-24 px-4 text-white'>
      <div className='max-w-2xl mx-auto'>
        ...
        <p className='text-gray-400 mb-8'>
          Boek een PC of PS5. Let op de regels.
        </p>

        <form
          onSubmit={handleSubmit}
          className='bg-slate-900 border border-slate-800 p-4 md:p-8 rounded-3xl space-y-6 shadow-xl'
        >
            ...
            <div className='bg-slate-950 p-4 rounded-xl border border-slate-800'>
              ...
              <div className='flex gap-2'>
                {Array.from({
                  length: Math.min(
                    4,
                    formData.inventory === 'switch'
                      ? inventory['Nintendo Controllers'] || 0
              ...
              {formData.date && !checkAvailability(1) && (
                <p className='text-xs text-red-500 mt-2'>
                  Geen controllers meer beschikbaar op deze datum.
                </p>
              )}
            </div>
          )}
            ...
            <div className='flex items-center gap-3 bg-slate-950 p-4 rounded-xl border border-slate-800'>
              <button
                ...
                className={`w-10 h-6 rounded-full p-1 transition-colors ${
                  formData.extraController ? 'bg-red-600' : 'bg-slate-700'
                }`}
              >
                <div
                  className={`w-4 h-4 bg-white rounded-full transition-transform ${
                    formData.extraController ? 'translate-x-4' : ''
                  }`}
                />
              </button>
          ...
          <div className='p-4 bg-slate-950 rounded-xl border border-slate-800'>
            ...
            {formData.date && availableStartTimes.length === 0 && (
              <p className='text-xs text-red-400 mt-2'>
                Geen beschikbare sloten voor deze selectie.
              </p>
            )}
          </div>
          ...
          <div className='flex items-start gap-3'>
            ...
            <label htmlFor='terms' className='text-sm text-gray-400'>
              ...
              Bij schade wordt mijn studentenaccount belast.
            </label>
          </div>
