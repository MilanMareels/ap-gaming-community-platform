      ...
      <div className='container mx-auto'>
        <ScrollReveal direction='up'>
          ...
          <p className='text-gray-400 text-center mb-12'>
            Onze competitieve studenten.
          </p>
        </ScrollReveal>

        {games.length === 0 ? (
          <div className='text-center text-gray-500 py-12 border border-dashed border-slate-800 rounded-xl max-w-md mx-auto'>
          ...
          <>
            ...
            {/* Player Cards */}
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-[repeat(auto-fit,16rem)] justify-center gap-6 max-w-6xl mx-auto'>
              {players.length === 0 ? (
                  ...
                  <ScrollReveal key={entry.id} direction='up' delay={idx * 50}>
                    <div className='h-full bg-slate-900 border border-slate-800 rounded-2xl p-6 hover:border-red-500/50 hover:-translate-y-2 transition-all group text-center flex flex-col justify-between'>
                      <div>
                        ...
                        <h3
                          className='text-xl font-black text-white truncate px-2'
                          title={entry.handle}
                        >
                          {entry.handle}
                        </h3>
                      ...
                      <div className='space-y-2'>
                        {entry.role && (
                          <div className='flex justify-between text-xs p-2 bg-slate-950 rounded border border-slate-800/50'>
                            ...
                            <span className='font-bold truncate max-w-25 text-right'>
                              {entry.role}
                            </span>
                          </div>
                        )}
                        {entry.rank && (
                          <div className='flex justify-between text-xs p-2 bg-slate-950 rounded border border-slate-800/50'>
                            ...
                            <span className='font-bold text-red-500 truncate max-w-25 text-right'>
                              {entry.rank}
                            </span>
                          </div>
                        )}
